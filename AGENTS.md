...
- **Pre-existing test failure:** `src/App.test.js` contains the default CRA placeholder test that searches for "learn react" text, which does not exist in the actual app. Running `npm test` will show 1 failing test — this is expected and pre-existing.
