        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          {events.filter((e) => e.status === 'closed').length > 0 && (
            <button
              onClick={() => {
                if (
                  window.confirm(`Archive all ${events.filter((e) => e.status === 'closed').length} closed events?`)
                ) {
                  setEvents((prev) =>
                    prev.map((event) => (event.status === 'closed' ? { ...event, status: 'archived' } : event))
                  );
                }
              }}
              style={{
                ...
                >
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <p style={{ fontSize: '18px', fontWeight: 'bold', margin: '0', color: '#f59e0b' }}>
                      {
                        communications.filter((c) => c.recipients.includes(event.name) || c.recipients === 'All Events')
                          .length
                      }
                    </p>
