...
const PaymentProcessingModal = ({ paymentFormData, setPaymentFormData, onClose, onSubmit }) => {
  return (
    <div
      role="dialog"
      aria-modal="true"
                ...
                <>
                  {paymentFormData.paymentMethod === 'stripe' && (
                    <p style={{ margin: '0 0 4px 0' }}>
                      Fee: ${(parseFloat(paymentFormData.amount) * 0.029 + 0.3).toFixed(2)}
                    </p>
                  )}
                  {paymentFormData.paymentMethod === 'square' && (
                    <p style={{ margin: '0 0 4px 0' }}>
                      Fee: ${(parseFloat(paymentFormData.amount) * 0.026 + 0.1).toFixed(2)}
                    </p>
                  )}
                  {paymentFormData.paymentMethod === 'paypal' && (
                    <p style={{ margin: '0 0 4px 0' }}>
                      Fee: ~${(parseFloat(paymentFormData.amount) * 0.029 + 0.49).toFixed(2)}
                    </p>
                  )}
                  {paymentFormData.paymentMethod === 'ach' && <p style={{ margin: '0 0 4px 0' }}>Fee: $0.25</p>}
                  {paymentFormData.paymentMethod === 'plaid' && <p style={{ margin: '0 0 4px 0' }}>Fee: $0.25</p>}
                </>
              )}
            </div>
          </div>
